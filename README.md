...
- **Model tournament** (`npm run tournament`) — the selector layer. Instead
  ...
  contestants per site — online expanding-mean base-rate null (zero future
  lookahead), recency-window(50) null, logistic regression, gradient boosting
  (25 and 50 trees), and a 3-symbol pattern miner (27 states) —
  ...
  live Brain loads it with zero changes; a NO_SIGNAL or INSUFFICIENT_DATA
  verdict immediately updates `model-verdict-<site>.json` and retires any
  old deployed model so the live Brain falls back to discipline-only right
  away. `data/tournament-verdict-<site>.json` keeps the full standings as an
  audit trail, and the startup log surfaces the last result.
- **Dependence & Microstructure Laboratory** (`npm run research:dependence`) —
  a dedicated non-ML laboratory for testing whether the crash series exhibits
  ANY non-random serial structure after uniformizing the marginal distribution:
  1. *Probability Integral Transform (PIT) & Ljung-Box Test:* transforms raw
     crashes into $U(0,1)$ variates under the fair null $S_0(x) = (1-r)/x$ and
     tests for serial correlation in the uniform domain.
  2. *Multi-representation Autocorrelation:* computes lag-1..5 autocorrelation
     across raw multipliers, $\log(X)$, $PIT(X)$, and binary exceedance indicators
     $1(X \ge 1.30)$ and $1(X \ge 2.00)$.
  3. *Markov Transition Matrix:* tests 3-state (tercile) and 5-state (quintile)
     transition probability matrices with $\chi^2$ independence tests.
  4. *Shannon Mutual Information & Conditional Entropy:* measures $I(X_t; X_{t+1})$
     and $H(X_{t+1} \mid X_t)$ with permutation significance tests.
  5. *Wald-Wolfowitz Runs Test:* checks whether clustering above/below the
     median occurs more or less frequently than expected under independence.
- **Distributional Research Laboratory** (`npm run research:distribution`) —
  instead of reducing the problem to a single binary target (e.g. $P(X \ge 1.30)$),
  measures the entire empirical survival curve $S(x)$ across 14 target points
  ($1.01\text{x} \dots 50\text{x}$) and tests whether conditional distributions
  (after a low crash, after a high win, after a cold loss streak) differ from
  the unconditional curve using 2-sample Kolmogorov-Smirnov ($D$) and
  Wasserstein (Earth Mover's) distance tests with exact permutation p-values.
- **Strip Reconciliation (Zero Dropped Rounds):** the game monitor reconciles the
  entire on-screen payout strip against the previous observation, automatically
  reconstructing and recovering all intermediate rounds even when multiple
  rounds crash between polling ticks.
- **In-flight Microstructure Recording:** captures flight trajectory samples
  ($t, v$), total flight duration, and time-to-target milestones (1.2x, 1.5x, 2.0x)
  into permanent per-site trace logs (`data/traces-<site>.jsonl`).
