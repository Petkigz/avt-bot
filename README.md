...
- **Deterministic prediction pairing:** every prediction generated by the engine
  receives a unique `predictionId` and `roundId`. Settle records explicitly
  reference the same ID, ensuring 1:1 deterministic pairing for calibration,
  error analysis and tournament datasets without heuristic key matching.
